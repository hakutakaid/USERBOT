...
async def uptotelegraph(client: Client, message: Message):
    ...
    if message.reply_to_message.media:
        ...
        try:
            ...
            return
        U_done = (
            f"`Berhasil Di Upload Ke` [Telegraph](https://telegra.ph/{media_url[0]})"
        )
        await tex.edit(U_done, disable_web_page_preview=True)
        os.remove(m_d)
        ...
        try:
            ...
            await tex.edit(f"**ERROR:** `{exc}`")
            return
        wow_graph = f"`Di Upload Ke` [Telegraph](https://telegra.ph/{response['path']})"
        await tex.edit(wow_graph, disable_web_page_preview=True)