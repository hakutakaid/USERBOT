...
My Github : https://github.com/Hakutaka1234
THANKS TO 
CREDIT BY @T0M1_X 
CREDIT BY @Kenapanan
"""
...
@Client.on_message(filters.me & filters.command("video", cmd))
async def yt_video(client, message):
    ...
    infomsg = await basic(message, "<code>Processing...</code>")
    try:
        search = (
            SearchVideos(
...
@Client.on_message(filters.me & filters.command("song", cmd))
async def yt_audio(client, message):
    ...
    await client.send_audio(
        ...
        file_name=title,
        duration=duration,
        caption=f"<b>ᴜᴘʟᴏᴀᴅ ʙʏ :</b> {client.me.mention}",
    )
    await infomsg.delete()
    for files in (thumbnail, file_path):
        if files and os.path.exists(files):
